...
// PFN: optional setting to unlock wallet for block minting only;
...
// PFN: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // PFN: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // PFN: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // PFN: not avatar mode
                    {
...
// PFN: create coin stake transaction
...
// PFN: check 'spent' consistency between wallet and txindex
// PFN: fix wallet spent state according to txindex
...
// PFN: disable transaction (only for coinstake)
